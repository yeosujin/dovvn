...
- 재시작 스케줄러(`src/main/restartScheduler.ts`, `src/main/restart.ts`) 추가. 다운로드 큐가 빌 때까지 재시작을 미루고, 대기 중에는 `download:start`를 차단한다.
- 앱 교체 전에 앱이 있는 폴더의 쓰기 권한을 점검해, 교체가 반복 실패할 때 자동 재시작이 되풀이되지 않게 했다.
- `startDownload`가 동기 예외를 던지면 항목이 큐에 영구히 남던 문제 수정.
