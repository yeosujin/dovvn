...
- 업데이트가 있으면 상단에 "지금 재시작" 버튼이 떠요. 다운로드가 끝나길 기다리지 않고 바로 재시작할 수 있어요 (진행 중인 다운로드는 중단되며, 몇 개가 중단되는지 버튼에 표시돼요)
...
서브에이전트 실행 중 코드 리뷰에서 나온 지적을 다음 커밋으로 반영했다. 스펙 문서에 모두 반영되어 있다.

| 지적                                                                                                                       | 처리                                                                                                                        |
| -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Important: `onPendingChange?.(true)` 호출이 예외 처리 없이 노출돼, 리스너가 예외를 던지면 `pending`이 영구히 `true`로 굳음 | `restartScheduler.ts`에 `notifyPending` 헬퍼를 추가해 두 호출 지점 모두 try/catch로 감쌈. 테스트 2개 추가                   |
| Important: yt-dlp 자동 확인 실패가 완전히 조용히 묻혀 영구 고장이 나도 알 방법이 없음                                      | `console.warn('[updater] 자동 업데이트 확인 실패', e)` 추가                                                                 |
| Important: `forceRestart()`가 `pending`이 이미 `false`일 때 호출되면 `forcing`이 영구히 고정될 수 있음                     | `if (!get().pending) return` 가드 추가, `Promise.all` → `Promise.allSettled`로 교체                                         |
| Important: 강제 재시작 중(`forcing`) 버튼 문구가 그대로라 최대 3.5초까지 멈춘 것처럼 보임                                  | `forcing`일 때 "재시작 중…"으로 문구 교체, 아이콘도 `animate-pulse`에서 `animate-spin`으로 통일                             |
| Important: 파괴적 동작(다운로드 취소)이 호버 툴팁에만 있어 클릭 전에 못 볼 수 있음                                         | 원래 사용자 결정("툴팁 또는 그에 준하는 문구")의 허용 범위 안에서, 다운로드가 있을 때 버튼 문구 자체에도 개수를 표시        |
| Minor: `phase === 'downloaded'`를 스피너 분기에서 빼지 않으면 드문 실패 경로에서 필이 조용히 잘못된 문구를 보여줌          | (구현 중에는 놓쳤다가) **전체 브랜치 최종 리뷰에서 발견** — `checking`/`available`/`downloading` 조건에서 `downloaded` 제거 |

마지막 항목은 개별 작업 리뷰가 아니라 전체 브랜치 리뷰(Task 8 이후)에서 잡혔다. 개별 작업 리뷰가 그 커밋이 바꾼 부분(문구·아이콘)에만 집중하느라, 몇 줄 아래 그대로 남아 있던 조건을 놓쳤다.

## 알려진 한계 (스펙과 동일)

전체 목록은 스펙 문서의 "범위 밖 (알려진 한계)"를 따른다. 요약:

- 창을 완전히 닫아둔 사이에는 앱 업데이트 주기 확인이 멈춘다(렌더러 타이머). yt-dlp 쪽은 메인 타이머라 이 한계가 없다.
- "지금 재시작" 버튼을 눌렀을 때 취소되는 다운로드는 되돌릴 수 없다. 버튼 문구와 툴팁 경고만 제공한다.
- yt-dlp 자동 확인과 버전 조회(`ytdlp:version`)가 겹치면 드물게 파이썬 트레이스백이 노출될 수 있다(기존 코드, 이번에 건드리지 않음). 최종 설치 버전에는 영향 없음.
- 이전 스펙(`2026-09-21-auto-restart-after-update-design.md`)의 알려진 한계도 그대로 유효하다.

## 스모크 테스트로 검증하지 못한 것

- 6시간 주기 자체, 앱 업데이트(electron-updater) 경로의 자동 재시작(실제 GitHub 릴리즈 필요), 다운로드 2개 이상일 때 버튼 문구, 창을 닫았다 다시 열었을 때 `restart:is-pending` 조회로 대기 상태가 복원되는지. 마지막 항목은 이 기능에서 새로 추가한 IPC 중 실행 증거가 없는 유일한 경로다.
