...
| 6   | 시나리오 2에서 다운로드가 끝나 재시작된 뒤 다운로드 목록 확인                           | 방금 끝난 마지막 다운로드가 `완료`로 남아 있다 (`실패 / 앱 종료로 중단됨`으로 바뀌면 렌더러 `localStorage` 저장이 종료 전에 끝나지 않은 것이다)       |
...
- 쓰기 권한 사전 점검을 통과한 뒤 교체 스크립트 자체가 실패하면 반복 재시작이 남을 수 있고, 실패 로그는 `/tmp/dovvn-update.log`에만 남는다.
- 재시작 대기 중 yt-dlp "업데이트 확인"을 다시 누르면 "이미 최신 버전입니다"가 표시되어 재시작 예정 안내가 사라진다. 대기 자체는 유지된다.
- 이 계획 초안에 있던 "설치 실패 시 `UpdatePill`이 `downloaded`에 갇힘" 한계는 아래 리뷰 수정으로 해소했다.

전체 목록은 스펙 문서의 "범위 밖 (알려진 한계)"를 따른다.

## 구현 중 반영한 리뷰 수정

서브에이전트 실행 중 코드 리뷰에서 나온 지적을 다음 커밋으로 반영했다. 스펙 문서(`docs/superpowers/specs/2026-09-21-auto-restart-after-update-design.md`)에 모두 반영되어 있다.

| 지적                                                                                                                               | 처리                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Critical: 교체가 반복 실패하면 캐시된 업데이트가 남아 실행할 때마다 자동 재시작이 되풀이됨                                         | `canReplaceApp()`으로 앱 폴더 쓰기 권한을 사전 점검하고, 없으면 압축 해제 전에 오류 표시 후 `false` 반환 (사용자 결정: 시도 기록 방식은 채택 안 함) |
| 설치 중 예외가 나면 `app-update:error`가 가지 않아 UI가 갇힘                                                                       | `installDownloadedUpdate()` 본문 전체(`!downloadedFile` 확인 포함)를 try/catch로 감싸 오류 전달, `false` 반환                                       |
| 종료 지연 300ms가 클릭 없는 자동 재시작에는 짧음 (마지막 다운로드 상태 저장과 안내 문구)                                           | `QUIT_DELAY_MS = 1_500`                                                                                                                             |
| 설치 실패 시 스토어가 `downloaded`에 갇혀 필이 계속 표시되고 이후 업데이트 확인도 멈춤                                             | `appUpdateStore`가 `downloaded` 상태에서도 `app-update:error`를 반영                                                                                |
| `startDownload`가 동기 예외를 던지면 항목이 `active`에 남아 재시작이 영영 실행되지 않고 다운로드도 계속 차단됨                     | `drain()`에서 예외를 잡아 `wrapped.onError`로 실패 처리 (사용자 결정: 이번 브랜치에 포함)                                                           |
| 멈춘 다운로드가 재시작을 무기한 미룰 수 있음                                                                                       | 대기 시간 제한을 두지 않음 (사용자 결정). 취소 버튼으로 풀 수 있고 `download:cancel`은 차단하지 않음                                                |
| 오류 문구: `null`/`undefined`를 던지면 `catch` 안에서 다시 예외가 남, 이미 `/Applications`에 있는 일반 사용자에게 안내가 막다른 길 | `e instanceof Error ? e.message : String(e)`, 문구를 "쓸 수 있는 위치로 옮기거나 관리자에게 문의"로 수정                                            |
| `onIdle` 주석이 실제(유휴로 끝날 때마다 호출)와 다르게 읽힘                                                                        | 주석 정정                                                                                                                                           |
